...
def add_row_in_get_df_results(df,trial_app,app,model_args,metrics):
    ...
    for trial_fold in best_model_names:
        ...
        pattern_2 = re.compile(rf"{trial_app}_\d+_f")
        if pattern_1.search(trial_fold):
            ...
            pattern_2_i = re.compile(rf"{trial_app}_\d+_f\d+$")
...
def get_df_results(trial_id,model_args,L_Apps,metrics = ['mse','mae','mape','mase'],name_id=None):
    ...
    if L_Apps == []:
        ...
        app = name_id
        df = add_row_in_get_df_results(df,trial_app,app,model_args,metrics)
    else:
        for app in L_Apps:
