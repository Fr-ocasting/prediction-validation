...
def plot_conformal_bands(preds,Y_true,station_i, q,pi,conformity_scores,window_pred = np.arange(2*96),bins = 20):
    ...
    ax1.fill_between(grid,lower,upper, label = f"PI: {'{:.2f}'.format(q.item())}-th empirical quantile: {'{:.0f}'.format(pi.Q.item())} \n PICP: {'{:.2%}'.format(pi.picp)} \n MPIW: {'{:.2f}'.format(pi.mpiw)}",alpha=0.6, color='#86cfac')
    ...
    ax2.plot([pi.Q,pi.Q],[0,0.2],color = 'red', linestyle = 'dashed',label = f"Q={'{:.0f}'.format(pi.Q.item())} is the {'{:.2f}'.format(q.item())}-th quantile")
