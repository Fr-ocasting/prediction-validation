...
class AttentionLayer(nn.Module):
    ...
    def __init__(self, model_dim, num_heads=8, mask=False):
        ...
        assert self.model_dim % self.num_heads == 0, (f"model_dim {self.model_dim} must be divisible by num_heads {self.num_heads}")
...
class DSTRformer(nn.Module):
    ...
    def forward(self,  x: Tensor,
        ...
        # x: [B,C,N,L]
        # x_calendar: [B,L,2] (dayofweek, timeofday)
        # x_vision: None
        ...
        if x_calendar is None:
            ...
            if self.node_dim > 0:
                ...
                elif len(self.adj_mx) == 1:
                    node_forward = self.adj_mx[0].to(self.device)
                    node_forward = self.adj_mx_forward_encoder(node_forward.unsqueeze(0)).expand(batch_size, self.in_steps, -1,
                                                                                                -1)
                    graph = torch.cat([adp_graph, node_forward], dim=-1)
                else:
                    raise ValueError(f"adj_mx should be a list of 1 or 2 adjacency matrices but here {len(self.adj_mx)}.")
