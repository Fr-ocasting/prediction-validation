...
# Personnal import:
import pipeline.Flex_MDI.dl_models.STGCN.STGCN_layer as layers
from pipeline.Flex_MDI.dl_models.MTGNN.MTGNN_layer import graph_constructor
        ...
        for ds_name in args.contextual_kwargs.keys():
            if 'attn_kwargs' in args.contextual_kwargs[ds_name].keys() and 'attn_late' in args.contextual_kwargs[ds_name]['attn_kwargs'].keys():
                if args.contextual_kwargs[ds_name]['attn_kwargs']['attn_late']:
                    script = importlib.import_module(f"pipeline.Flex_MDI.dl_models.SpatialAttn.SpatialAttn")
                    scrip_args = importlib.import_module(f"pipeline.Flex_MDI.dl_models.SpatialAttn.load_config")
