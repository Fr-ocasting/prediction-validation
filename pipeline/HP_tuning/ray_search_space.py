...
def get_search_space_ray(args):
    ...
    config = {"lr": tune.loguniform(5e-5, 5e-3), # tune.qloguniform(1e-5, 1e-2, 1e-5), # tune.qloguniform(5e-5, 5e-3, 5e-5)
            ...
            # if 'torch_scheduler_type' == 'MultiStepLR',
                ...
                #    {"torch_scheduler_milestone": tune.choice([[5,10,15],[10,20,25],[25, 45, 65], ... 
                #    "torch_scheduler_gamma": tune.uniform(0.98, 0.995), "torch_scheduler_lr_start_factor": tune.uniform(0.3, 0.9)}
    ...
    module_path = f"pipeline.dl_models.{args.model_name}.search_space"
   ...
   # Tackle Embedding
    if len(vars(args.args_embedding))>0:
        module_path = f"pipeline.dl_models.TimeEmbedding.search_space"
        search_space_module = importlib.import_module(module_path)
        importlib.reload(search_space_module)
    ...
    if hasattr(args, 'args_vision') and len(vars(args.args_vision)) > 0:
        module_path = f"pipeline.dl_models.vision_models.{args.args_vision.model_name}.search_space"
        search_space_module = importlib.import_module(module_path)
        importlib.reload(search_space_module)
