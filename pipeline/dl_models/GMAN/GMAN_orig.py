...
"""from https://github.com/VincLee8188/GMAN-PyTorch """
...
if __name__ == '__main__':
    ...
    # 1. Configuration pour reproduire l'environnement
    seed = 42
    torch.manual_seed(seed)
    np.random.seed(seed)

    # Paramètres basiques
    B, N, L, T_pred = 2, 10, 12, 12
    C = 1 # GMAN original suppose implicitement C=1 ou C inclu dans l'input
    K, d = 2, 4 # Petites dimensions pour le test
    D = K * d
    
    # 2. Mock des arguments
    ...
    # 3. Génération des données factices
    # X: [Batch, His, Nodes]
    X = torch.randn(B, L, N)
    
    # TE: [Batch, His + Pred, 2] (Time of day, Day of week)
    # On simule des entiers pour le one-hot encoding interne
    TE = torch.randint(0, 7, (B, L + T_pred, 2)).float() 

    # SE: [Nodes, D] (Spatial Embedding fourni en entrée dans l'original)
    SE = torch.randn(N, D)

    print("-" * 30)
    print("Test GMAN Original")
    print("-" * 30)
    
    # 4. Initialisation et Forward
    model = GMAN(SE, args, bn_decay=0.1)
    
    # Le modèle original est souvent en mode train par défaut, on met en eval pour fixer le dropout
    model.eval() 
    ...
    print(f"Output shape: {output.shape}") # Devrait être [B, T, N]
    print(f"Output sum (Check): {output.sum().item():.4f}")
    print(f"Output mean (Check): {output.mean().item():.4f}")
    print("-" * 30)