'''LSTM EncoderDecoder from https://github.com/JulesBelveze/time-series-autoencoder/tree/master
an implementation of the code of this paper https://arxiv.org/pdf/1704.02971

This repository contains an autoencoder for multivariate time series forecasting. 
It features two attention mechanisms described in 
A Dual-Stage Attention-Based Recurrent Neural Network for Time Series Prediction 
and was inspired by Seanny123's repository. '''
...
def init_hidden(x: torch.Tensor, hidden_size: int, num_dir: int = 1, xavier: bool = True, device: str = 'cpu'):
    """
    Initialize hidden.
...
class Encoder(nn.Module):
    def __init__(self, args, input_size: int):
        ...
        self.lstm = nn.LSTM(input_size=input_size, hidden_size=args.hidden_size_encoder)
    ...
    def forward(self, input_data: torch.Tensor):
        ...
        Args:
            input_data: shape : [B,L,C] ??
        """
        h_t, c_t = (init_hidden(input_data, self.hidden_size, device = self.device),
                    init_hidden(input_data, self.hidden_size, device = self.device))
        input_encoded = Variable(torch.zeros(input_data.size(0), self.seq_len, self.hidden_size))

        for t in range(self.seq_len):
            # input_data[:, t, :].unsqueeze(0) of shape: [1,B,H], with H = self.hidden_size
...
class AttnEncoder(nn.Module):
    ...
    def forward(self, input_data: torch.Tensor):
        ...
        Args:
            input_data: (torch.Tensor): tensor of input data
        """
        h_t, c_t = (init_hidden(input_data, self.hidden_size, num_dir=self.directions, device = self.device),
                    init_hidden(input_data, self.hidden_size, num_dir=self.directions, device = self.device))
        ...
        for t in range(self.seq_len):
            x = torch.cat((h_t.repeat(self.input_size, 1, 1).permute(1, 0, 2),
                           ...
                           input_data.permute(0, 2, 1).to(self.device)), dim=2).to(
                self.device)  # bs * input_size * (2 * hidden_dim + seq_len)
...
class Decoder(nn.Module):
    def __init__(self, args):
        ...
        self.lstm = nn.LSTM(1, args.hidden_size_decoder, bidirectional=False)
        self.fc = nn.Linear(args.hidden_size_decoder, args.output_size)
    ...
    def forward(self, _, y_hist: torch.Tensor):
        ...
        """
        h_t, c_t = (init_hidden(y_hist, self.hidden_size, device = self.device),
                    init_hidden(y_hist, self.hidden_size, device = self.device))
...
class AttnDecoder(nn.Module):
    def __init__(self, args):
        ...
        self.out_feats = args.output_size
    ...
    def forward(self, input_encoded: torch.Tensor, y_history: torch.Tensor):
        ...
        """
        h_t, c_t = (init_hidden(input_encoded, self.decoder_hidden_size, device = self.device), 
                    init_hidden(input_encoded, self.decoder_hidden_size, device = self.device))
        context = Variable(torch.zeros(input_encoded.size(0), self.encoder_hidden_size))

        for t in range(self.seq_len):
            x = torch.cat((h_t.repeat(self.seq_len, 1, 1).permute(1, 0, 2),
                           c_t.repeat(self.seq_len, 1, 1).permute(1, 0, 2),
                           input_encoded.to(self.device)), dim=2)
            ...
            y_tilde = self.fc(torch.cat((context.to(self.device), y_history[:, t].to(self.device)),
...
class AutoEncForecast(nn.Module):
    def __init__(self, args, input_size):
        ...
        self.encoder = AttnEncoder(args, input_size).to(args.device) if args.input_att else Encoder(args, input_size).to(args.device)
        self.decoder = AttnDecoder(args).to(args.device) if args.temporal_att else Decoder(args).to(args.device)
