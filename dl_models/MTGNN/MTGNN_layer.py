...
class graph_constructor(nn.Module):
    ...
    def forward(self, idx):
        ...
        mask.scatter_(1,t1,s1.fill_(1).to(dtype = mask.dtype))
        adj = adj*mask
        return adj
...
class LayerNorm(nn.Module):
    ''' Objet 'layer_norm'. Initialise les poids à W = 1, b = 0.  
    Utilise une simple torch.nn.functional(layer_norm)

    ajouts d'autres fonctionalité : reset les paramètres
    '''
...
class dilated_inception(nn.Module):
    '''
    Consiste en 4 'temporal convolution' (nn.Conv2d), de plus ou moins grande fenêtre : kernel = (1,2), (1,3), (1,6), et (1,7)

    Args :
    ---------
    - cin : identique pour tout le monde
    - cout : identique pour tout le monde, et égale au cout total divisé par le nombre de temporal conv
    - dilation : 2


    Sortie :
    ---------
    - Tronque la dernière axis de toutes les temporal-conv à la même dimension (du kernel le plus grand)  (x[i] = x[i][...,-x[-1].size(3):]). 
    - concatène toute les temporal conv
    '''
...
class mixprop(nn.Module):
    ...
    Args: 
    ------
    - c_in,c_out : dimension de la mlp de sortie
    - gdep : nombre de graph conv identique que l'on va concaténer.
    - dropout : inutile ici
